...
from training.projectors import w_projector, w_plus_projector
...
class BaseCoach:
    ...
    def calc_inversions(self, image, image_name,w_pivot,text_samples,clip_loss):
        ...
        if hyperparameters.first_inv_type == 'w+':
            ...
            w = w_projector.project(self.G, id_image,w_pivot,text_samples,clip_loss, device=torch.device(global_config.device), w_avg_samples=600,
    ...
    def calc_loss(self, generated_images, real_images, log_name, new_G, use_ball_holder, w_batch):
        ...
        if hyperparameters.id_loss_param > 0:
            loss_id, sim_improvement, id_logs = self.id_loss(generated_images, real_images)
            # loss_dict['loss_id'] = float(loss_id)
            # loss_dict['id_improve'] = float(sim_improvement)
            loss += loss_id * hyperparameters.id_loss_param        

        return loss, l2_loss_val, loss_lpips, loss_id
