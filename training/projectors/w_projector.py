...
def project(
    ...
    for step in tqdm(range(num_steps)):
        ...
        print('loading from predicted latents')
        # w_noise = torch.randn_like(w_opt) * w_noise_scale
        ws = (w_opt + 0.2 * pred_w).repeat([1, G.mapping.num_ws, 1])
        # ws = w_noise
        # print(ws.shape)
        synth_images = G.synthesis(ws, noise_mode='const', force_fp32=True)
        
        clip_losses = clip_loss(synth_images,text_samples)
    ...
    layers_ = G.mapping.num_ws
    del G
    return (w_opt+0.2 * pred_w).repeat([1, layers_, 1])